...
public interface PostMatchRepository extends JpaRepository<Claim, Long> {
    ...
    @Query(
            ...
            SELECT
                ...
                COALESCE(cma.USER_PEND, 'N')                AS pended,
                em.MEMBER_ID                                AS insuredId,
                em.LAST_NAME                                AS lastName,
                em.FIRST_NAME                               AS firstName,
                CAST(em.DT_OF_BIRTH AS DATE)                AS dateOfBirth,
                em.GENDER                                   AS gender,
                em.POLICY                                   AS policy,
                c.CLAIM_TYPE                                AS claimType,
                c.CLAIM_STREAM                              AS network,
                CAST(cma.CLAIM_RECEIVED_AT AS DATE)         AS claimReceivedDate
            FROM CLAIM c
            JOIN CLIENT_MATCH_ACTION cma
                 ON c.MPI_CLAIM_ID = cma.EDP_CLAIM_ID
            ...
            WHERE 1=1
              ...
              AND (:clientClaimId IS NULL OR UPPER(c.CLIENT_CLAIM_ID) LIKE UPPER(CONCAT(:clientClaimId, '%')))
              AND (:insuredId    IS NULL OR UPPER(em.MEMBER_ID)      LIKE UPPER(CONCAT(:insuredId, '%')))
              AND (:lastName     IS NULL OR UPPER(em.LAST_NAME)      LIKE UPPER(CONCAT(:lastName, '%')))
              AND (:firstName    IS NULL OR UPPER(em.FIRST_NAME)     LIKE UPPER(CONCAT(:firstName, '%')))
              AND (:dob          IS NULL OR CAST(em.DT_OF_BIRTH AS DATE) = :dob)
              AND (:gender       IS NULL OR UPPER(em.GENDER)         = UPPER(:gender))
              AND (:policy       IS NULL OR UPPER(em.POLICY)         LIKE UPPER(CONCAT(:policy, '%')))
              AND (:ccode        IS NULL OR UPPER(cma.MATCHED_CLIENT_CODE) = UPPER(:ccode))
              AND (:network      IS NULL OR UPPER(c.CLAIM_STREAM)    = UPPER(:network))
              AND (:matchedBy    IS NULL OR UPPER(cma.MODIFIED_BY)   LIKE UPPER(CONCAT(:matchedBy, '%')))
              AND (:matchType    IS NULL OR UPPER(r.MATCH_TYPE)      = UPPER(:matchType))
              AND (:claimType    IS NULL OR UPPER(c.CLAIM_TYPE)      = UPPER(:claimType))
              AND (:dateFrom     IS NULL OR CAST(cma.CLAIM_RECEIVED_AT AS DATE) >= :dateFrom)
              AND (:dateTo       IS NULL OR CAST(cma.CLAIM_RECEIVED_AT AS DATE) <= :dateTo)
            ORDER BY cma.CLAIM_RECEIVED_AT DESC, c.MPI_CLAIM_ID DESC
        """,
            countQuery = """
            SELECT COUNT(1)
            FROM CLAIM c
    ...
    @Query(value = """
        SELECT
            cma.EDP_CLAIM_ID                   AS claimId,
            cma.MATCHED_CLIENT_CODE            AS ccode,
            r.SCENARIO                         AS scenario,
            r.MATCH_TYPE                       AS matchType,
            COALESCE(cma.MATCHED_MEMBER_ID, em.MEMBER_ID) AS memberId,
        ...
        LEFT JOIN PEND_NOTE pn
             ON pn.CLAIM_NUMBER = cma.EDP_CLAIM_ID
        WHERE cma.EDP_CLAIM_ID = :claimId
